...
// Imports
// ==================================
var http = require('http'); // its a default package of node.js
...
var removeDiacritics = require('diacritics').remove; // global.fetch = require("node-fetch");


var mime = require('mime'); // const { exec } = require("child_process");
      ...
      switch (_context2.prev = _context2.next) {
        ...
        case 6:
          ...
          if (!sensorExist.length) {
            ...
            res.redirect('/map/zone?zoneid=' + userData[0].zoneId); // res.send({
            //     ...getQuery,
...
app.get('/api/v3/get-sensor-data', function (req, res) {
  ...
  // Process time
  var todayRaw = new Date(); // todayRaw.setDate(todayRaw.getDate()-1)
  ...
  if (['door'].includes(req.query.type)) {
    var _todayRaw = new Date();

    _todayRaw.setDate(_todayRaw.getDate() - 1);
...
app.get('/api/v3/set-sensor-name', function (req, res) {
  sess = req.session;
  ...
  if (sess.username) {
    ...
    var spaceCount = req.query.name.split(" ").length - 1;
...
var PORT = process.env.PORTANYSENSOR; // const PORT = 3008

var server = app.listen(PORT, console.log("NodeJS started on port ".concat(PORT))).on('error', function (err) {
  console.log(err);
