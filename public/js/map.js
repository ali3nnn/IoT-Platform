    ...
    (async () => {
        ...
        if (length == 0) {
            ...
            for (var zone = 0; zone < length; zone++) {
                const htmlRes = `<div class="zone-item">
                    <a href="/map/${json.result[zone].county}" county="${json.result[zone].county}" class="county-item"><i class="fas fa-map-marker-alt"></i><span>${json.result[zone].county}</span></a>
                </div>`
                $('#zones-list').append(htmlRes);
            }
